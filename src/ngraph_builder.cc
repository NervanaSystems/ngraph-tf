...
enum class QuantConvOps {
  QuantizedConv2DWithBiasSumAndReluAndRequantize,
  QuantizedConv2DWithBiasSignedSumAndReluAndRequantize,
  QuantizedConv2DWithBiasAndReluAndRequantize,
  QuantizedConv2DWithBiasAndRequantize
};
template <QuantConvOps OpType>
static Status QuantizedConvHelper(
    const Node* op, const std::vector<const Tensor*>& static_input_map,
    Builder::OpMap& ng_op_map) {
  ...
  bool has_sum_input =
      (OpType ==
       QuantConvOps::QuantizedConv2DWithBiasSumAndReluAndRequantize) ||
      (OpType ==
       QuantConvOps::QuantizedConv2DWithBiasSignedSumAndReluAndRequantize);
  if (has_sum_input) {
    TF_RETURN_IF_ERROR(GetInputNode(ng_op_map, op, 9, &ng_sum_input));
  }

  vector<size_t> static_inps_idxs;
  if (has_sum_input) {
    static_inps_idxs = {3, 4, 5, 6, 7, 8, 10, 11};
  } else {
    static_inps_idxs = {3, 4, 5, 6, 7, 8};
  }
  ...
  std::vector<std::shared_ptr<ng::op::Constant>> static_inps(
      static_inps_idxs.size());
  for (int i = 0; i < static_inps.size(); i++) {
    std::vector<float> tmp_vect;
    TF_RETURN_IF_ERROR(GetStaticInputVector(op, static_inps_idxs[i],
                                            static_input_map, &tmp_vect));
    if (tmp_vect.size() != 1) {
      return errors::InvalidArgument((opnames[static_cast<int>(OpType)],
                                      " Op: Input number ", static_inps_idxs[i],
                                      " must be scalar. Got a vector of size, ",
                                      tmp_vect.size()));
    }
  ...
  BatchToNGraph(is_nhwc, ng_input);
  if (has_sum_input) {
    BatchToNGraph(is_nhwc, ng_sum_input);
  }
  auto& ng_filter_shape = ng_filter->get_shape();
  ng_kernel_shape[0] = ng_filter_shape[0];
  ng_kernel_shape[1] = ng_filter_shape[1];
  ...
  Builder::MakePadding(tf_padding_type, ng_image_shape, ng_kernel_shape,
                       ng_strides, ng_dilations, ng_padding_below,
                       ng_padding_above);
  // It is expected by the builder functions that the min max inputs be constant
  // nodes
  // Hence declaring them static, reading their values and converting to
  // constant nodes
  std::shared_ptr<ng::Node> ng_quant_conv;
  switch (OpType) {
    case QuantConvOps::QuantizedConv2DWithBiasSumAndReluAndRequantize:
      ng_quant_conv = ng::builder::ScaledQuantizedConvolutionBiasAdd(
      ...
      break;
    case QuantConvOps::QuantizedConv2DWithBiasSignedSumAndReluAndRequantize:
      ng_quant_conv = ng::builder::ScaledQuantizedConvolutionBiasSignedAdd(
          ng_input, ng_filter, ng_bias, ng_sum_input, ng_strides, ng_dilations,
          ng_padding_below, ng_padding_above, ng_data_dilations, static_inps[0],
          static_inps[1], static_inps[2], static_inps[3], static_inps[4],
          static_inps[5], static_inps[6], static_inps[7], true);
      break;
    case QuantConvOps::QuantizedConv2DWithBiasAndReluAndRequantize:
    case QuantConvOps::QuantizedConv2DWithBiasAndRequantize:
      ng_quant_conv = ng::builder::ScaledQuantizedConvolutionBias(
          ng_input, ng_filter, ng_bias, ng_strides, ng_dilations,
          ng_padding_below, ng_padding_above, ng_data_dilations, static_inps[0],
          static_inps[1], static_inps[2], static_inps[3], static_inps[4],
          static_inps[5],
          OpType == QuantConvOps::QuantizedConv2DWithBiasAndReluAndRequantize);
      break;
    default:
      return errors::InvalidArgument("Got incorrect QuantConvOps");
  }
  BatchToTensorflow(is_nhwc, ng_quant_conv);
  SaveNgOp(ng_op_map, op->name(), ng_quant_conv);
...
const static std::map<
    ...
    TRANSLATE_OP_MAP{
        ...
        {"QuantizedConv2DWithBiasAndReluAndRequantize",
         QuantizedConvHelper<
             QuantConvOps::QuantizedConv2DWithBiasAndReluAndRequantize>},
        {"QuantizedConv2DWithBiasAndRequantize",
         QuantizedConvHelper<
             QuantConvOps::QuantizedConv2DWithBiasAndRequantize>},
        {"QuantizedConv2DWithBiasSignedSumAndReluAndRequantize",
         QuantizedConvHelper<
             QuantConvOps::
                 QuantizedConv2DWithBiasSignedSumAndReluAndRequantize>},
