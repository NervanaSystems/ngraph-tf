...
//std::shared_ptr<ng::Node> CreateQuantizedConvNode(std::vector<>){

//}



static Status TranslateQuantizedConv(const Node* op, const std::vector<const Tensor*>& static_input_map,
    Builder::OpMap& ng_op_map, std::function<std::shared_ptr<ng::Node>(std::vector<std::shared_ptr<ng::Node>>, ng::Strides, ng::Strides, ng::CoordinateDiff, ng::CoordinateDiff, ng::Strides, std::vector<std::shared_ptr<ng::op::Constant>>)> create_quantized_conv_node){
  size_t num_tf_op_inputs = op->num_inputs();
  std::vector<int32> static_inps_idxs;
  if (GetNodeAttr(op->attrs(), "_ngraph_static_inputs", &static_inps_idxs) != Status::OK()){
    return errors::Internal("Did not find static inputs in quantized convolution op");
  }
  ...
  // It is expected by ScaledQuantizedConvolutionBias (and other builder functions) that the min max inputs be constant nodes
