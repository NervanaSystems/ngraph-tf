...
//   tracker->MarkFresh(tensor_base_ptr, ng_func1); // marks "t" fresh for ng_func1
//
...
class NGraphFreshnessTracker : public ResourceBase {
 public:
  ...
  // If freshness_map_ has the base_pointer, then inserts the user function into its set of user functions     
  void MarkFresh(const void* base_pointer,
                 std::shared_ptr<ngraph::Function> user);

  // Checks if the freshness_map_ has the user function for base_pointer, else returns false
  bool IsFresh(const void* base_pointer,
               std::shared_ptr<ngraph::Function> user);

  // Removes all the functions for the base_pointer in the freshness_map_, i.e. sets the set<ng::Function> for base_pointer to empty
  void MarkStale(const void* base_pointer);
  
  // Inserts the base_pointer in the map, initialises the set of functions as empty set
 ...
 // mutex protecting the freshness_map_
  mutex mu_;
  // for each base pointer (of tensor), maintains a set of ng::Functions that use it
