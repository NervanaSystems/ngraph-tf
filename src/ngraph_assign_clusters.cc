...
Status AssignClusters(Graph* graph) {
  ...
  // cluster, then all we have to do is add a 2 'shadow' edges and 1 'shadow' node
  ...
  // cycle). The contraction only happens on 'real' edges (edges that are present
  // in the TF graph itself). Therefore the shadow edges in the gc data structure
  // will never suffer contraction. Anytime the shadow path's src and dst attempt
  // a merge (by contracting some real edge between them), the shadow path will
  // introduce a cycle and not allow it
  ...
  // then, since we have broken the contract that an edge in gc implies an edge in
  // TF in this fix
   for (auto node : graph->op_nodes()) {
    ...
    if (static_inputs.size() > 0) {
      ...
      for (auto static_inp_idx : static_inputs) {
        ...
        if (static_edge->src()->type_string() != "Const"){
          ...
          gc.InsertEdge(cluster_map[static_edge->src()]->index, shadow_node_index);
          gc.InsertEdge(shadow_node_index, cluster_map[static_edge->dst()]->index);
        }
      }
    }
  }
