...
def sanitize_node_names(graph_def):
    ...
    return modify_node_names(graph_def, {
                             node.name: node.name[1:] for node in graph_def.node if node.name[0] == "_"})
...
def prepend_to_name(graph_def, node_map):
    ...
    '''
    return modify_node_names(graph_def, {
                             node.name: node_map[node.name] + node.name for node in graph_def.node if node.name in node_map})


def load_file(graph_file, input_binary, modifier_function_list=[]):
    '''
    can load and modify protobuf (pb or pbtxt)
    '''
    ...
    with open(graph_file, "r") as f:
        ...
        try:
            ...
            raise Exception(
                "Failed to read pb or pbtxt. input_binary is " +
                str(input_binary) +
...
def preprocess(input_filename, out_dir, input_binary, node_map):
    ...
    # Else sanitize_node_names might change the node names, which might become
    # unrecognizable to node_map
    modifiers = [
        lambda pbtxt_str: prepend_to_name(
            pbtxt_str,
            node_map),
        sanitize_node_names]
...
def graphdef_to_tensorboard(gdef, tensorboard_output):
    ...
    # this is because FileWriter has deprecated graphdef as inputs, and
    # prefers graphs as inputs
    with tf.Session() as sess:
        ...
        # TODO: How about supporting NGraphVariable and NGraphEncapsulateOp by
        # switching their optype with something TB knows
        writer.flush()
        writer.close()
    # It seems NGraphVariable and NGraphEncapsulateOp are registered in C++
    # but not in python
    print("\n" + ('=-' * 30) + "\nTo view Tensorboard:")
...
def protobuf_to_grouped_tensorboard(
        input_filename,
        tensorboard_dir,
        input_binary=False,
        node_map={}):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c",
        "--cluster",
        help="An file that contains the node-to-cluster map that can be used to group them into clusters")
