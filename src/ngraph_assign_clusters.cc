...
Status AssignClusters(Graph* graph) {
  ...
  // cluster, then all we have to do is add a 2 'shadow' edges and 1 'shadow'
  // node
  ...
  // present
  // in the TF graph itself). Therefore the shadow edges in the gc data
  // structure
  // will never suffer contraction. Anytime the shadow path's src and dst
  // attempt
