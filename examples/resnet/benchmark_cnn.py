...
class BenchmarkCNN(object):
  ...
  def _eval_once(self, saver, summary_writer, target, local_var_init_op_group,
    ...
    with tf.Session(
      ...
      Ref_5 = 0.9003
      Ref_count = 49920
      log_fn('Reference Accuracy @ 1 = %.4f Reference Accuracy @ 5 = %.4f [%d examples]' %
             (Ref_1, Ref_5, Ref_count))
      Valid_1 = 1.0 - np.absolute(accuracy_at_1-Ref_1)/Ref_1
      Valid_5 = 1.0 - np.absolute(accuracy_at_5-Ref_5)/Ref_5
      log_fn('Validation @ 1 = %.4f Validation @ 5 = %.4f [%d examples] (1.0 is the ideal case )' %
             (Valid_1, Valid_5, Ref_count))
      assert Valid_1 >= 0.95
      assert Valid_5 >= 0.95 
  ...
  def _build_fetches(self, global_step, all_logits, losses, device_grads,
    ...
    for d, device in enumerate(apply_gradient_devices):
      with tf.device('/job:localhost/replica:0/task:0/device:'+self.params.select_device+':0'):
