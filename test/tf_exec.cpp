...
TEST(tf_exec, DISABLED_BatchMatMul_0D) {
  ...
  AssertTensorEqualsFloat(outputs_z1[0], outputs_z1_cpu[0]);
  AssertTensorEqualsFloat(outputs_z2[0], outputs_z2_cpu[0]);
  AssertTensorEqualsFloat(outputs_z[0], outputs_z_cpu[0]);
}
...
TEST(tf_exec, BatchMatMul) {
  ...
  AssertTensorEqualsFloat(outputs_z1_ng[0], outputs_z1_tf[0]);
  AssertTensorEqualsFloat(outputs_z2_ng[0], outputs_z2_tf[0]);
}
...
TEST(tf_exec, DISABLED_BatchMatMul_2D) {
  ...
  ASSERT_OK(sess.Run({C}, &outputs_cpu));
  ASSERT_EQ(outputs[0].shape(), outputs_cpu[0].shape());
  AssertTensorEqualsFloat(outputs[0], outputs_cpu[0]);
}
...
TEST(tf_exec, DISABLED_FusedBatchNormGrad_NHWC) {
  ...
  AssertTensorEqualsFloat(outputs[0], outputs_cpu[0]);
  AssertTensorEqualsFloat(outputs[1], outputs_cpu[1]);
  AssertTensorEqualsFloat(outputs[2], outputs_cpu[2]);
}
...
TEST(tf_exec, DISABLED_Op_L2Loss) {
  ...
  for (auto const& input_size : input_sizes) {
    ...
    AssertTensorEqualsFloat(outputs_ngraph[0], outputs_cpu[0]);
  }
}
...
TEST(tf_exec, DISABLED_Op_Unpack) {
  ...
  for (auto i = 0; i < input_sizes.size(); ++i) {
    ...
    for (auto j = 0; j < input_rank; ++j) {
      ...
      AssertTensorEqualsFloat(outputs_ngraph[j], outputs_cpu[j]);
    }
  }
}
...
TEST(tf_exec, DISABLED_Tile) {
  ...
  AssertTensorEqualsFloat(outputs_C[0], outputs_C_cpu[0]);
  AssertTensorEqualsFloat(outputs_D[0], outputs_D_cpu[0]);
}
...
TEST(tf_exec, DISABLED_Op_PreventGradient) {
  ...
  AssertTensorEqualsFloat(outputs_cpu[0], outputs_ng[0]);
}
