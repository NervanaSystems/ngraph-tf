...
* A json file: Provide model specific parameters. Look at the example ```mnist_cnn.json``` for graph and ```mnist_mlp.json``` for checkpoint. You can start with the ```template.json``` and modify it to match your model
